...
def Open_Check(ws, response, data, server):
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            time.sleep(int(data['wait_error']))
            continue
    ...
    while True:
        ...
        def main(wss):
            try:
                response = Receive(wss)
            except:
                ws = Connect(server)
                response = Receive(ws)
            Open_Check(ws, response, data, server)
        ...
        if server == old_server and data == old_data:
            try:
                ...
                Log2(" >>> Cập nhật lại config sau " + data['update_config'] + " giây!")
        else:
